...
#     data_to_encode = (filename_size.to_bytes(4, 'big') + filename + compressed_data)
...
# Server URL to retrieve the partial phrase
SERVER_URL = "https://example.com/partial_phrase"
MAX_ATTEMPTS = 3  # Maximum allowed attempts to retrieve the partial phrase
RATE_LIMIT_PERIOD = 60  # Rate limit period in seconds
RATE_LIMIT_ATTEMPTS = 5  # Maximum allowed attempts within the rate limit period
...
# def retrieve_partial_phrase():
#     attempts = 0
#     while attempts < MAX_ATTEMPTS:
#         try:
#             response = requests.get(SERVER_URL)
#             if response.status_code == 200:
#                 return response.text.strip()  # Assuming the server returns the partial phrase as text
#             else:
#                 print("Failed to retrieve partial phrase from the server")
#                 return None
#         except requests.RequestException as e:
#             print(f"Failed to retrieve partial phrase from the server: {e}")
#             attempts += 1
#             time.sleep(1)  # Add a delay between attempts to prevent excessive retries
